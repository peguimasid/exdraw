...
- User clicks "Continue with GitHub" → OAuth popup → auto-creates account → lands on `/draw`
- If account exists, just logs them in
- Beautiful button styling with brand colors and icons

---

### **Phase 2: Basic Collaborative Drawing** 🎨
...
**How it should work:**
- User moves mouse on canvas with button pressed → smooth black line appears instantly
...
**How it should work:**
- Select Rectangle tool → click and drag → see preview rectangle → release → rectangle created
- Hold Shift while dragging circle → perfect circle
- Hold Shift while drawing line → snaps to 15° increments
...
**How it should work:**
- Click Select tool → click any drawn element → blue bounding box appears
- Click and drag element → it moves smoothly
- Click canvas background → deselects
- Press Delete/Backspace → removes selected element
...
- Click color → popup shows grid of preset colors (red, blue, green, yellow, orange, purple, pink, black, gray, white)
- Click color → applies to current tool
...
- "Clear Canvas" in menu (or Ctrl+Shift+Delete) → confirmation modal → clears everything
...
**How it should work:**
- User draws line → presses Ctrl+Z → line disappears
- Press Ctrl+Shift+Z → line reappears
...
**How it should work:**
- User clicks menu → "Export as PNG"
...
**How it should work:**
- User on `/draw` → auto-creates/joins default room
- Click "New Canvas" → generates random ID → redirects to `/draw/abc123`
- Copy URL → send to friend → friend opens URL → both drawing on same canvas
...
**How it should work:**
- User draws elements → they're saved to database automatically
- User closes browser → comes back later → all drawings still there
...
**How it should work:**
- Open on iPhone → toolbar appears at bottom as horizontal row
- Draw with finger → smooth lines
- Pinch with two fingers → zooms canvas in/out
...
- Can invite friend via URL → both draw together in real-time
