defmodule ExdrawWeb.UserLive.Registration do
  use ExdrawWeb, :live_view

  alias Exdraw.Accounts
  alias Exdraw.Accounts.User
    ...
    <div class="flex h-dvh bg-base-200 items-center justify-center">
      <div class="w-full max-w-md">
        ...
        <div class="rounded-2xl shadow-lg bg-base-100 p-8">
          ...
          <div class="mb-6">
            <h2 class="text-2xl font-semibold text-base-content mb-2">Create your account</h2>
            <p class="text-sm text-base-content/60">
              Get started with exdraw today
            </p>
          </div>
          ...
          <div class="mt-6 text-center text-sm text-base-content/60">
            ...
            <.link navigate={~p"/users/log-in"} class="text-primary hover:underline font-medium">
              Sign in
            </.link>
          </div>
        </div>
  ...
  def mount(_params, _session, %{assigns: %{current_scope: %{user: user}}} = socket)
    ...
    {:ok, redirect(socket, to: ExdrawWeb.UserAuth.signed_in_path(socket))}
  end
